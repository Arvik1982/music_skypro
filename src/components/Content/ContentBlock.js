...
export function Content({activeTrack, setActiveTrack,  setPlayerOn}) {
  ...
  useEffect(()=>{getTracks().then((data)=>{errorText=null;tracks = data;setContentVisible(true); return tracks})
  .catch((error) =>{errorText=error.message;setContentVisible(true); tracks=[]; return errorText })},[])
   ...
   return (
    <S.CentralBlockContent>
      ...
      <S.CentralBlockContentPlaylist>
        ...
        <div style={{color:'red'}}><h1>{errorText!==null?`Ошибка: ${errorText}, попробуйте позже`:null}</h1></div>
        ...
        {tracks.map((track)=>{return(
          ...
          >
            ...
            <S.Track_time>
              ...
              {contentVisible ? (
                <S.Track__timeText>{track.duration_in_seconds}</S.Track__timeText>
              ) : (
                <SkeletonTheme baseColor="#202020" highlightColor="#444">
